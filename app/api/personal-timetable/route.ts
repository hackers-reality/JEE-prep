...
async function readPayload(payload: string | null | undefined): Promise<PersonalPayload> {
  ...
  try {
    const parsed = JSON.parse(payload) as Partial<PersonalPayload>;
    return {
      rows: Array.isArray(parsed.rows) ? parsed.rows : [],
      logs: parsed.logs && typeof parsed.logs === "object" ? parsed.logs : {},
      syllabus: parsed.syllabus && typeof parsed.syllabus === "object" ? parsed.syllabus : {},
    };
  } catch {
    return emptyPayload;
  }
...
export async function GET(request: Request) {
  ...
  try {
    if (share) {
      ...
      return NextResponse.json({ ok: true, mode: "viewer", payload: await readPayload(row.payload), updatedAt: row.updatedAt, studentName: row.name ?? "JEE 2028 student" });
    }
    ...
    return NextResponse.json({ ok: true, mode: "owner", payload: await readPayload(row?.payload), shareToken: row?.shareToken ?? null, updatedAt: row?.updatedAt ?? null });
  } finally {
    client.close();
  }
}
...
export async function PUT(request: Request) {
  ...
  const payload: PersonalPayload = {
    ...
    logs: body.logs && typeof body.logs === "object" ? body.logs as Record<string, unknown> : {},
    syllabus: body.syllabus && typeof body.syllabus === "object" ? body.syllabus as Record<string, string> : {},
  };
  await ensureTable();
  const client = db();
  try {
    ...
    await client.execute({
      sql: `INSERT INTO "PersonalTimetable" (studentId, payload, shareToken, updatedAt) VALUES (?, ?, ?, CURRENT_TIMESTAMP)
            ON CONFLICT(studentId) DO UPDATE SET payload = excluded.payload, updatedAt = CURRENT_TIMESTAMP`,
      args: [student.id, JSON.stringify(payload), shareToken],
    });
...
export async function POST() {
  ...
  try {
    ...
    await client.execute({
      sql: `INSERT INTO "PersonalTimetable" (studentId, payload, shareToken, updatedAt) VALUES (?, ?, ?, CURRENT_TIMESTAMP)
            ON CONFLICT(studentId) DO UPDATE SET shareToken = excluded.shareToken, updatedAt = CURRENT_TIMESTAMP`,
      args: [student.id, payload, shareToken],
    });
