...
const ROUTINE: RoutineItem[] = [
  ...
  { id: "coaching", time: "16:30–21:15", title: "Coaching", desc: "Class + travel buffer. Mark doubts and teacher-selected problems.", subject: "life", duration: 285, study: false, editable: false },
  ...
  { id: "break", time: "23:15–23:30", title: "Short break", desc: "No doom-scrolling.", subject: "life", duration: 15, study: false, editable: false },
  ...
  { id: "log", time: "01:00–01:30", title: "Daily log + doubt journal", desc: "Questions, wrong/unattempted GT items, doubts, notes.", subject: "life", duration: 30, study: false, editable: false },
  { id: "catchup", time: "01:30–03:00", title: "Buffer / catch-up / question finishing", desc: "Finish the target if needed; otherwise light revision.", subject: "life", duration: 90, study: true, editable: true },
...
function defaultLog(): Log { return { q: { phy: 0, chem: 0, math: 0 }, routine: {}, notes: "", gtWrong: 0, gtUnattempted: 0, doubtsCleared: 0 }; }
function defaultPayload(): Payload { return { rows: ROUTINE, logs: {}, syllabus: {} }; }
function subjectLabel(s: Subject | "life") { return s === "life" ? "Life" : SUBJECT_META[s].label; }
...
function dayScore(log: Log) { const hourScore = Math.min(1, studyHours(log) / DAILY_HOUR_TARGET); const qScore = Math.min(1, totalQuestions(log) / DAILY_Q_TARGET); const adherenceScore = adherence(log); return Math.max(0, Math.min(1, hourScore * 0.4 + qScore * 0.3 + adherenceScore * 0.3)); }
function subjectScore(log: Log, subject: Subject) {
  const qScore = Math.min(1, log.q[subject] / 18);
  const hasRoutine = ROUTINE.some((r) => r.study && r.subject === subject && log.routine[r.id]);
  const scheduleScore = hasRoutine ? 1 : 0;
  return Math.max(0, Math.min(1, qScore * 0.65 + scheduleScore * 0.35));
}
function heatStyle(score: number) {
  const alpha = 0.08 + score * 0.84;
  return { backgroundColor: `rgba(61,220,255,${alpha})`, boxShadow: score > 0.15 ? `inset 0 0 0 1px rgba(61,220,255,${Math.min(0.7, alpha + 0.08)})` : undefined };
}
...
export default function PersonalTimetablePage() {
  ...
  const todayQ = totalQuestions(log);
  const todayHrs = studyHours(log);
  const todayAdherence = adherence(log);
  const todayPlanQ = planQuestions(currentDate);
    ...
    if (share) {
      ...
      fetch(`/api/personal-timetable?share=${encodeURIComponent(share)}`)
        ...
        .then((data) => {
          ...
          if (payload?.syllabus) setSyllabus(payload.syllabus);
  ...
  async function saveAll(nextLogs = logs, nextSyllabus = syllabus) {
    ...
    try {
      const response = await fetch("/api/personal-timetable", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ payload }) });
  ...
  function updateLog(patch: Partial<Log>, persist = true) {
    const next = { ...log, ...patch } as Log;
    const nextLogs = { ...logs, [currentDate]: { ...next, savedAt: new Date().toISOString() } };
    setLogs(nextLogs);
    setSyncState("idle");
    if (persist) void saveAll(nextLogs, syllabus);
  }

  function toggleRoutine(id: string) {
    if (viewer) return;
    const nextRoutine = { ...log.routine, [id]: !log.routine[id] };
    updateLog({ routine: nextRoutine });
  }

  async function share() { await saveAll(); }

  function toggleSyllabus(key: string) {
    if (viewer) return;
    const current = syllabus[key] ?? "todo";
    ...
    void saveAll(logs, nextSyllabus);
  }
  ...
  let streak = 0;
  for (let i = progressDays.length - 1; i >= 0; i--) {
    const d = progressDays[i].data;
    if (d && totalQuestions(d) >= 40) streak++; else break;
  }

  const plannedStudyBlocks = ROUTINE.filter((r) => r.study).length;
  const doneStudyBlocks = ROUTINE.filter((r) => r.study && log.routine[r.id]).length;
  const subjectTouch = (s: Subject) => ROUTINE.some((r) => r.study && r.subject === s && log.routine[r.id]) || log.q[s] > 0;
  ...
  return <main className="min-h-screen bg-[#0B0D12] text-[#E7E9EE]"><div className="mx-auto max-w-[980px] px-4 pb-20 pt-5 sm:px-6">
    ...
    {activeView === "today" && <div className="space-y-4">
      <section className="rounded-2xl border border-[#232838] bg-[#12151C] p-5">
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-4"><SmallStat label="GT wrong" value={log.gtWrong}/><SmallStat label="GT unattempted" value={log.gtUnattempted}/><SmallStat label="Doubts cleared" value={log.doubtsCleared}/><SmallStat label="Plan" value={todayPlanQ || "—"}/></div>
        <div className="mt-4"><label className="text-[10px] uppercase tracking-wider text-[#565D70]">Notes / stuck points</label><textarea disabled={viewer} value={log.notes} onChange={e=>updateLog({notes:e.target.value}, false)} onBlur={()=>void saveAll()} placeholder="What did you cover? Which doubt goes to teacher? Why was something skipped?" className="mt-1 min-h-[76px] w-full rounded-lg border border-[#232838] bg-[#171B24] px-3 py-2 text-sm outline-none disabled:opacity-60"/></div>
      ...
      <section className="rounded-2xl border border-[#232838] bg-[#12151C] p-5"><div className="mb-3 flex items-center justify-between"><div><h2 className="text-[15px] font-semibold">Today's routine</h2><p className="text-[11px] text-[#565D70]">Tick a study block when it is actually completed. Study hours calculate automatically.</p></div><div className="text-right"><div className="font-mono text-lg font-bold text-[#4ADE80]">{todayHrs.toFixed(2)} h</div><div className="text-[9px] uppercase tracking-wider text-[#565D70]">of {DAILY_HOUR_TARGET} h target</div></div></div>
        <div className="mb-4 h-2 overflow-hidden rounded-full bg-[#171B24]"><div className="h-full rounded-full bg-[#3DDCFF] transition-all" style={{width:`${Math.min(100, todayAdherence*100)}%`}}/></div>
        <div className="space-y-0">{ROUTINE.map(item=>{const checked=!!log.routine[item.id];return <label key={item.id} className={`flex gap-3 border-b border-[#232838] py-3 last:border-b-0 ${item.study?'cursor-pointer':'cursor-default'}`}><div className="w-[80px] shrink-0 font-mono text-[11px] text-[#565D70]">{item.time}</div><div className="pt-0.5"><input type="checkbox" disabled={viewer || !item.editable} checked={checked} onChange={()=>toggleRoutine(item.id)} className="h-4 w-4 accent-[#3DDCFF] disabled:opacity-50"/></div><div className="mt-1 h-2 w-2 shrink-0 rounded-full" style={{background:item.subject==='life'?'#565D70':SUBJECT_META[item.subject].color}}/><div className="flex-1"><div className={`text-[13px] font-semibold ${checked?'text-[#E7E9EE]':'text-[#8B92A5]'}`}>{item.title}{item.study&&<span className="ml-2 text-[10px] font-normal text-[#565D70]">{Math.floor(item.duration/60)}h {item.duration%60?`${item.duration%60}m`:''}</span>}</div><div className="text-[12px] text-[#565D70]">{item.desc}</div></div>{checked&&<div className="text-xs font-bold text-[#4ADE80]">✓</div>}</label>})}</div>
      </section>
    </div>}
...
function ScheduleView() {
  return <div className="space-y-4">
    <section className="rounded-2xl border border-[#232838] bg-[#12151C] p-5"><h2 className="mb-1 text-[15px] font-semibold">Day-by-day plan → GT-02</h2><p className="mb-4 text-[12px] text-[#565D70]">New topics move forward while completed topics stay alive through revision and application.</p>{DAILY_PLAN.map(p=>{const today=isoToday();const past=p.date<today;const current=p.date===today;const total=p.items?.reduce((s,i)=>s+i.q,0)??0;return <div key={p.date} className={`border-b border-[#232838] py-3 last:border-b-0 ${past?'opacity-40':''} ${current?'rounded-lg bg-[#3DDCFF]/5 px-3':''}`}><div className="mb-1 flex items-center gap-2 text-[12px] font-bold">{fmtDate(p.date)} {current&&<span className="rounded-full bg-[#3DDCFF] px-2 py-0.5 text-[9px] text-[#031014]">TODAY</span>}</div>{p.revision?<div className="text-[12px] text-[#4ADE80]">🔁 {p.note}</div>:<div className="text-[12.5px] leading-6">{p.items?.map((i,idx)=><span key={idx}><span style={{color:i.sub==='rev'?'#4ADE80':SUBJECT_META[i.sub].color}}>{i.sub==='rev'?'REV':SUBJECT_META[i.sub].label}</span>: {i.t} ({i.q}){idx<(p.items?.length??1)-1?' · ':''}</span>)} <strong> · = {total} Qs</strong></div>}</div>})}</section>
    <section className="rounded-2xl border border-[#232838] bg-[#12151C] p-5"><h2 className="mb-3 text-[15px] font-semibold">Exact daily routine</h2>{ROUTINE.map(item=><div key={item.id} className="flex gap-3 border-b border-[#232838] py-2.5 last:border-b-0"><div className="w-[90px] shrink-0 font-mono text-[11px] text-[#3DDCFF]">{item.time}</div><div className="flex-1 text-[13px]">{item.title}<div className="text-[11px] text-[#8B92A5]">{item.desc}</div></div>{item.study&&<div className="font-mono text-[11px] text-[#4ADE80]">{(item.duration/60).toFixed(2)}h</div>}</div>)}</section>
    <section className="rounded-2xl border border-[#232838] bg-[#12151C] p-5"><h2 className="mb-3 text-[15px] font-semibold">Non-negotiables</h2>{['40–60 questions is the working daily range; 50 is the aim.','6–7 hours self-study separate from coaching.','Touch Physics, Chemistry and Maths every day.','Clear Level 1 teacher-selected external-book questions before moving up.','Chemistry gets slightly more weight.','Analyse wrong + unattempted GT questions and reattempt them.'].map(x=><div key={x} className="flex gap-2 border-b border-[#232838] py-2.5 text-[12.5px] last:border-b-0"><span className="text-[#3DDCFF]">•</span><span>{x}</span></div>)}</section>
  </div>;
}

function ProgressView({ logs, progressDays, totalQ, avgQ, streak, totalHrs }:{logs:Record<string,Log>;progressDays:{iso:string;data:Log|null}[];totalQ:number;avgQ:number;streak:number;totalHrs:number}) {
  const avgAdherence = progressDays.filter(d=>d.data).length ? progressDays.filter(d=>d.data).reduce((s,d)=>s+adherence(d.data as Log),0)/progressDays.filter(d=>d.data).length : 0;
  const last21 = progressDays.slice(-21);
  const maxQ = Math.max(60, ...last21.map(d=>d.data?totalQuestions(d.data):0));
  return <div className="space-y-4">
    <div className="grid grid-cols-2 gap-3 md:grid-cols-5"><Metric value={totalQ} label="Total Qs" good={totalQ>0}/><Metric value={avgQ} label="Avg Q/day" good={avgQ>=DAILY_Q_TARGET}/><Metric value={streak} label="40+ streak" good={streak>=3}/><Metric value={totalHrs.toFixed(1)} label="Study hours" good={totalHrs>0}/><Metric value={`${Math.round(avgAdherence*100)}%`} label="Avg routine adherence" good={avgAdherence>=0.8}/></div>
    <section className="rounded-2xl border border-[#232838] bg-[#12151C] p-5"><h2 className="mb-1 text-[15px] font-semibold">Questions solved — last 21 days</h2><p className="mb-4 text-[11px] text-[#565D70]">Green band = 40–60 target. Bar opacity reflects how much of the routine was actually followed.</p><div className="grid grid-cols-21 gap-1 overflow-x-auto">{last21.map(d=>{const q=d.data?totalQuestions(d.data):0;const score=d.data?dayScore(d.data):0;const h=q?Math.max(6,Math.round((q/maxQ)*150)):4;return <div key={d.iso} className="flex min-w-[18px] flex-col items-center justify-end gap-1"><div className="text-[7px] text-[#565D70]">{q||''}</div><div className="w-full rounded-t-sm bg-[#3DDCFF]" style={{height:`${h}px`,opacity:0.12+score*0.88}} title={`${d.iso}: ${q} Q, ${d.data?studyHours(d.data).toFixed(2):0} h, ${d.data?Math.round(adherence(d.data)*100):0}% routine`}/><div className="h-1 w-full rounded bg-[#4ADE80]/20"/></div>})}</div></section>

    <section className="rounded-2xl border border-[#232838] bg-[#12151C] p-5"><div className="mb-2 flex items-end justify-between"><div><h2 className="text-[15px] font-semibold">Actual progress heatmap</h2><p className="text-[11px] text-[#565D70]">Intensity combines questions, self-study hours and timetable adherence. Dark = little/no progress. Bright = strong day.</p></div><div className="flex items-center gap-1 text-[9px] text-[#565D70]"><span>Low</span>{[0.05,0.25,0.45,0.65,0.85].map(v=><span key={v} className="h-3 w-3 rounded" style={heatStyle(v)}/>)}<span>High</span></div></div><div className="grid grid-cols-7 gap-1 sm:grid-cols-14 md:grid-cols-28">{progressDays.map(d=>{const score=d.data?dayScore(d.data):0;return <div key={d.iso} className="aspect-square rounded-[4px]" style={heatStyle(score)} title={`${d.iso} • ${d.data?totalQuestions(d.data):0} Q • ${d.data?studyHours(d.data).toFixed(2):0} h • ${d.data?Math.round(adherence(d.data)*100):0}% routine`}/>})}</div></section>

    <section className="rounded-2xl border border-[#232838] bg-[#12151C] p-5"><h2 className="mb-1 text-[15px] font-semibold">Subject intensity heatmap</h2><p className="mb-4 text-[11px] text-[#565D70]">Each subject gets its own intensity from questions plus whether its scheduled block was completed.</p><div className="space-y-3">{(['phy','chem','math'] as Subject[]).map(s=><div key={s}><div className="mb-1 flex justify-between text-[11px]"><span style={{color:SUBJECT_META[s].color}}>{SUBJECT_META[s].label}</span><span className="text-[#565D70]">{SUBJECT_META[s].book}</span></div><div className="grid grid-cols-14 gap-1">{progressDays.map(d=>{const score=d.data?subjectScore(d.data,s):0;const base=SUBJECT_META[s].color;return <div key={d.iso} className="aspect-square rounded-[4px]" title={`${d.iso} • ${d.data?d.data.q[s]:0} Q`} style={{backgroundColor:`${base}${Math.round(20+score*220).toString(16).padStart(2,'0')}`,boxShadow:score>0.3?`inset 0 0 0 1px ${base}55`:undefined}}/>})}</div></div>)}</div></section>

    <section className="rounded-2xl border border-[#232838] bg-[#12151C] p-5"><h2 className="mb-3 text-[15px] font-semibold">Recent notes & GT repair</h2>{progressDays.filter(d=>d.data && (d.data.notes || d.data.gtWrong || d.data.gtUnattempted || d.data.doubtsCleared)).slice(-10).reverse().map(d=><div key={d.iso} className="border-b border-[#232838] py-3 last:border-b-0"><div className="font-mono text-[10px] text-[#565D70]">{d.iso}</div><div className="mt-1 text-[12px] text-[#8B92A5]">{d.data?.notes || 'No notes'}</div><div className="mt-1 text-[10px] text-[#565D70]">GT wrong: {d.data?.gtWrong||0} · unattempted: {d.data?.gtUnattempted||0} · doubts: {d.data?.doubtsCleared||0}</div></div>)}{progressDays.every(d=>!d.data?.notes&&!d.data?.gtWrong&&!d.data?.gtUnattempted&&!d.data?.doubtsCleared)&&<div className="text-[12px] text-[#565D70]">No repair notes logged yet.</div>}</section>
  </div>;
}

function SyllabusView({ syllabus, onToggle, viewer }:{syllabus:Record<string,Status>;onToggle:(key:string)=>void;viewer:boolean}) {
  const entries = (Object.entries(SYLLABUS) as [Subject, typeof SYLLABUS[Subject]][]);
  const total = entries.reduce((s,[,v])=>s+v.topics.length,0);
  const done = entries.reduce((s,[sub,v])=>s+v.topics.filter(t=>(syllabus[`${sub}:${t.name}`]??t.status)==='done').length,0);
  return <div className="space-y-4"><section className="rounded-2xl border border-[#232838] bg-[#12151C] p-5"><h2 className="mb-2 text-[15px] font-semibold">GT-02 syllabus completion</h2><div className="h-2 overflow-hidden rounded-full bg-[#171B24]"><div className="h-full rounded-full bg-[#3DDCFF]" style={{width:`${(done/total)*100}%`}}/></div><div className="mt-2 text-[11px] text-[#565D70]">{done}/{total} topics complete</div></section>{entries.map(([sub,v])=><section key={sub} className="rounded-2xl border border-[#232838] bg-[#12151C] p-5"><h2 className="mb-3 text-[15px] font-semibold" style={{color:SUBJECT_META[sub].color}}>{SUBJECT_META[sub].label} <span className="ml-2 text-[10px] font-normal text-[#565D70]">{SUBJECT_META[sub].book}</span></h2>{v.topics.map(t=>{const key=`${sub}:${t.name}`;const status=syllabus[key]??t.status;return <button key={key} disabled={viewer} onClick={()=>onToggle(key)} className="mb-2 flex w-full items-center justify-between gap-3 rounded-xl border border-[#232838] bg-[#171B24] px-3 py-3 text-left disabled:cursor-default"><div><div className="text-[13px] font-semibold">{t.name}</div><div className="text-[10px] text-[#565D70]">{t.note}</div></div><span className={`rounded-full px-2.5 py-1 text-[9px] font-bold uppercase ${status==='done'?'bg-[#4ADE80]/15 text-[#4ADE80]':status==='progress'?'bg-[#FFB454]/15 text-[#FFB454]':'bg-[#565D70]/10 text-[#565D70]'}`}>{status}</span></button>})}</section>)}</div>;
}

function Metric({value,label,good}:{value:string|number;label:string;good:boolean}){return <div className="rounded-xl border border-[#232838] bg-[#171B24] p-3 text-center"><div className={`font-mono text-[22px] font-bold ${good?'text-[#4ADE80]':'text-[#E7E9EE]'}`}>{value}</div><div className="mt-1 text-[9px] uppercase tracking-wider text-[#565D70]">{label}</div></div>}
function SmallStat({value,label}:{value:number|string;label:string}){return <div className="rounded-lg border border-[#232838] bg-[#171B24] px-3 py-2"><div className="font-mono text-sm font-bold">{value}</div><div className="text-[9px] uppercase tracking-wider text-[#565D70]">{label}</div></div>}
