...
function ProgressView({progressDays,totalQ,avgQ,streak,totalHrs,avgAdherence}:{progressDays:{iso:string;data:Log|null}[];totalQ:number;avgQ:number;streak:number;totalHrs:number;avgAdherence:number}) {
  ...
  const plannedQ = progressDays.reduce((s,d) => s + planQuestions(d.iso), 0);
  const activeDays = logged.length;
  const plannedCompletion = plannedQ ? totalQ / plannedQ : 0;
  const maxQ = Math.max(60, ...last21.map(d => d.data ? totalQuestions(d.data) : 0));
  const maxHours = Math.max(DAILY_HOUR_TARGET, ...progressDays.map(d => d.data ? studyHours(d.data) : 0));
  const subjectTotals = (['phy','chem','math'] as Subject[]).map(s => ({ s, q: logged.reduce((n,d) => n + (d.data ? d.data.q[s] : 0), 0) }));
  const bestDay = logged.reduce<{iso:string;score:number;q:number;h:number} | null>((best,d) => { if (!d.data) return best; const candidate={iso:d.iso,score:dayScore(d.data),q:totalQuestions(d.data),h:studyHours(d.data)}; return !best || candidate.score > best.score ? candidate : best; }, null);
  const areaW = 920, areaH = 250, padL = 44, padR = 16, padT = 18, padB = 34;
  const innerW = areaW - padL - padR, innerH = areaH - padT - padB;
  const pts = progressDays.map((d,i) => { const h=d.data?studyHours(d.data):0; const x=padL+(i/(progressDays.length-1))*innerW; const y=padT+innerH-(h/maxHours)*innerH; return {x,y,h,d}; });
  ...
  const targetY = padT + innerH - (DAILY_HOUR_TARGET/maxHours)*innerH;
  const yTicks = [0, Math.round(maxHours/2*10)/10, Math.round(maxHours*10)/10];
  return <div className="space-y-4">
    <div className="grid grid-cols-2 gap-2 md:grid-cols-4 lg:grid-cols-8"><Metric value={totalQ} label="Qs / 28d"/><Metric value={totalHrs.toFixed(1)} label="Hours / 28d"/><Metric value={avgQ} label="Avg Q / day"/><Metric value={activeDays} label="Active days"/><Metric value={streak} label="40+ streak"/><Metric value={`${Math.round(avgAdherence*100)}%`} label="Routine avg"/><Metric value={`${Math.round(Math.min(1,plannedCompletion)*100)}%`} label="Plan completion"/><Metric value={bestDay ? fmtDate(bestDay.iso).replace(', 2026','') : '—'} label="Best day"/></div>

    <section className="rounded-2xl border border-[#232838] bg-[#12151C] p-5">
      <div className="mb-3 flex flex-wrap items-end justify-between gap-2"><div><h2 className="text-[15px] font-semibold">28-day study hours</h2><p className="text-[11px] text-[#565D70]">Area line = actual checked study time. The dashed line is the {DAILY_HOUR_TARGET} h daily target.</p></div><div className="font-mono text-[11px] text-[#8B92A5]">{totalHrs.toFixed(1)} h total</div></div>
      <div className="overflow-x-auto rounded-lg border border-[#232838] bg-[#0B0D12] p-2"><svg viewBox={`0 0 ${areaW} ${areaH}`} className="min-w-[720px] w-full" role="img" aria-label="28 day study hours area line graph">
        {yTicks.map((v,i) => { const y=padT+innerH-(v/maxHours)*innerH; return <g key={i}><line x1={padL} x2={areaW-padR} y1={y} y2={y} stroke="#232838" strokeWidth="1"/><text x={padL-8} y={y+4} textAnchor="end" fill="#565D70" fontSize="10">{v}h</text></g>; })}
        <line x1={padL} x2={areaW-padR} y1={targetY} y2={targetY} stroke="#4ADE80" strokeDasharray="5 5" strokeWidth="1" opacity="0.7"/>
        <path d={areaPath} fill="#A78BFA" fillOpacity="0.22"/>
        <path d={linePath} fill="none" stroke="#A78BFA" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"/>
        {pts.map((p,i) => <g key={p.d.iso}><circle cx={p.x} cy={p.y} r="3.5" fill="#A78BFA" stroke="#0B0D12" strokeWidth="2"><title>{`${fmtDate(p.d.iso)} • ${p.h.toFixed(2)} h`}</title></circle>{(i===0 || i===6 || i===13 || i===20 || i===27) && <text x={p.x} y={areaH-10} textAnchor={i===0?'start':i===27?'end':'middle'} fill="#565D70" fontSize="10">{new Date(`${p.d.iso}T12:00:00`).toLocaleDateString('en-US',{month:'short',day:'numeric'})}</text>}</g>)}
      </svg></div>
    </section>

    <section className="rounded-2xl border border-[#232838] bg-[#12151C] p-5"><div className="mb-3 flex items-end justify-between"><div><h2 className="text-[15px] font-semibold">Questions solved — last 21 days</h2><p className="text-[11px] text-[#565D70]">Rectangle bars are actual questions; opacity reflects the combined execution score.</p></div><span className="text-[10px] text-[#565D70]">Target 50 Q/day</span></div><div className="flex h-44 items-end gap-1 overflow-x-auto">{last21.map(d=>{const q=d.data?totalQuestions(d.data):0; const score=d.data?dayScore(d.data):0; const h=q?Math.max(8,Math.round((q/maxQ)*150)):4; return <div key={d.iso} className="flex h-full min-w-[20px] flex-1 flex-col items-center justify-end gap-1"><span className="text-[7px] text-[#565D70]">{q||''}</span><div className="w-full rounded-t-sm bg-[#3DDCFF]" style={{height:`${h}px`,opacity:0.12+score*0.88}} title={`${fmtDate(d.iso)}: ${q} Q, ${d.data?studyHours(d.data).toFixed(2):0} h, ${d.data?Math.round(adherence(d.data)*100):0}% routine`}/><div className="h-1 w-full rounded bg-[#4ADE80]/20"/></div>})}</div></section>

    <section className="rounded-2xl border border-[#232838] bg-[#12151C] p-5"><div className="mb-3 flex items-center justify-between"><div><h2 className="text-[15px] font-semibold">Execution heatmap</h2><p className="text-[11px] text-[#565D70]">Questions + self-study hours + timetable adherence. Bright means the day was actually executed.</p></div><div className="flex items-center gap-1 text-[9px] text-[#565D70]"><span>Low</span>{[0.05,0.25,0.45,0.65,0.85].map(v=><span key={v} className="h-3 w-3 border border-[#232838]" style={heatStyle(v)}/>)}<span>High</span></div></div><div className="grid grid-cols-7 gap-1 sm:grid-cols-14 md:grid-cols-28">{progressDays.map(d=>{const score=d.data?dayScore(d.data):0; return <div key={d.iso} className="aspect-square border border-[#232838]" style={heatStyle(score)} title={`${fmtDate(d.iso)} • ${d.data?totalQuestions(d.data):0} Q • ${d.data?studyHours(d.data).toFixed(2):0} h • ${d.data?Math.round(adherence(d.data)*100):0}% routine`}/>})}</div></section>

    <section className="rounded-2xl border border-[#232838] bg-[#12151C] p-5"><h2 className="mb-1 text-[15px] font-semibold">Subject intensity heatmaps</h2><p className="mb-4 text-[11px] text-[#565D70]">Each subject is computed independently from questions plus completion of its scheduled block.</p>{(['phy','chem','math'] as Subject[]).map(s=><div key={s} className="mb-4 last:mb-0"><div className="mb-1 flex justify-between text-[11px]"><span style={{color:SUBJECT_META[s].color}}>{SUBJECT_META[s].label}</span><span className="text-[#565D70]">{subjectTotals.find(x=>x.s===s)?.q ?? 0} Q · {SUBJECT_META[s].book}</span></div><div className="grid grid-cols-14 gap-1">{progressDays.map(d=>{const score=d.data?subjectScore(d.data,s):0; const c=SUBJECT_META[s].color; return <div key={d.iso} className="aspect-square border border-[#232838]" title={`${fmtDate(d.iso)} • ${d.data?d.data.q[s]:0} Q`} style={{backgroundColor:`${c}${Math.round(20+score*220).toString(16).padStart(2,'0')}`}}/>})}</div></div>)}</section>

    <section className="rounded-2xl border border-[#232838] bg-[#12151C] p-5"><div className="mb-3 flex items-center justify-between"><div><h2 className="text-[15px] font-semibold">28-day computed summary</h2><p className="text-[11px] text-[#565D70]">Everything below is derived from the same daily logs used by the graphs.</p></div><span className="text-[10px] text-[#565D70]">{plannedQ} planned Qs</span></div><div className="grid gap-2 md:grid-cols-4">{subjectTotals.map(({s,q})=><div key={s} className="border border-[#232838] bg-[#171B24] p-3"><div className="text-[10px] uppercase tracking-wider" style={{color:SUBJECT_META[s].color}}>{SUBJECT_META[s].label}</div><div className="mt-1 font-mono text-lg font-bold">{q} Q</div><div className="text-[10px] text-[#565D70]">{totalQ ? Math.round(q/totalQ*100) : 0}% of solved questions</div></div>)}<div className="border border-[#232838] bg-[#171B24] p-3"><div className="text-[10px] uppercase tracking-wider text-[#4ADE80]">Best execution day</div><div className="mt-1 font-mono text-sm font-bold">{bestDay ? fmtDate(bestDay.iso) : '—'}</div><div className="text-[10px] text-[#565D70]">{bestDay ? `${bestDay.q} Q · ${bestDay.h.toFixed(2)} h · ${Math.round(bestDay.score*100)}% score` : 'No logged day yet'}</div></div></div></section>
  </div>;
}
